...
def get_target_group_arn(target_group_key):
    target_groups = {
        ...
        "auth": [
            "arn:aws:elasticloadbalancing:us-east-1:723696830269:targetgroup/auth-Targe-example/arn-example"
        ],
        "devportal": [
            "arn:aws:elasticloadbalancing:us-east-1:723696830269:targetgroup/devportal-Targe-example/arn-example"
        ]
    }
...
def check_health(session, instance_id, service, region):
    # Define service-specific health check URLs
    health_check_urls = {
    ...
    # Check if the response contains 200 status
    if "200" in output:
        print(f"✅ Health check for {service} passed (status 200 OK).")
        return True
    else:
        print(f"❌ Health check for {service} failed. Status code not 200.")
        return False
...
def process_instances(session, target_group_arns, target_group_key, region):
    ...
    half_instances = len(instances) // 2 or 1  # Ensure at least one instance is processed
    instances_to_process = instances[:half_instances]  # Only process half to avoid downtime
    ...
    for instance_id in instances_to_process:
        ...
        # Add health check before re-registering
        ...
        if check_health(session, instance_id, target_group_key, region):
            ...
            time.sleep(2)  # Simulate re-registration
